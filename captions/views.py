...
def upload_image(request):
    ...
    if request.method == "POST" and request.FILES.get("image"):
        ...
        # Save uploaded image under static uploads so templates can access it (you can change to MEDIA later)
        upload_dir = os.path.join("captions", "static", "captions", "uploads")
        ...
        for lang_name, lang_code in langs.items():
            ...
            audio_files[lang_name] = text_to_speech(text or "", lang_code, filename)

        # 4) Paragraph & story
        paragraph = generate_paragraph(captions)
        story = generate_story(best_caption)

        # 5) Context for template (image path uses static folder)
        context = {
